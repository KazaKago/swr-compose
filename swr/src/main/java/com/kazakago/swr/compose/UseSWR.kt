...
@Composable
public fun <KEY, DATA> useSWR(
    key: () -> KEY?,
    fetcher: (suspend (key: KEY) -> DATA)? = null,
    scope: CoroutineScope? = null,
    options: SWRConfig<KEY, DATA>.() -> Unit = {},
): SWRState<KEY, DATA> {
    return useSWR(runCatching(key).getOrNull(), fetcher, scope, options)
}
...
public fun <KEY, DATA> useSWR(
    ...
    val config = SWRConfigImpl.from<KEY, DATA>(globalConfig).apply { options() }
